...
namespace Valve.VR.InteractionSystem
{
	...
	public class Teleport : MonoBehaviour
    {
		...
		private SteamVR_Player _steamVrPlayer = null;
		...
		void Update()
		{
			...
			foreach ( Hand hand in _steamVrPlayer.hands )
			{
				if ( visible )
				{
		...
		private void UpdatePointer()
		{
			...
			Vector3 playerFeetOffset = _steamVrPlayer.trackingOriginTransform.position - _steamVrPlayer.feetPositionGuess;
			...
			float dotForward = Vector3.Dot( pointerDir, _steamVrPlayer.hmdTransform.forward );
			...
			else //Hit neither
			{
				...
				float distanceFromPlayer = Vector3.Distance( hitInfo.point, _steamVrPlayer.hmdTransform.position );
		...
		private void ShowPointer( Hand newPointerHand, Hand oldPointerHand )
		{
			if ( !visible )
			{
				...
				foreach ( TeleportMarkerBase teleportMarker in teleportMarkers )
				{
					if ( teleportMarker.markerActive && teleportMarker.ShouldActivate( _steamVrPlayer.feetPositionGuess ) )
					{
				...
				startingFeetOffset = _steamVrPlayer.trackingOriginTransform.position - _steamVrPlayer.feetPositionGuess;
		...
		private void InitiateTeleportFade()
		{
			...
			headAudioSource.transform.SetParent( _steamVrPlayer.hmdTransform );
		...
		private void TeleportPlayer()
		{
			...
			{
				Vector3 playerFeetOffset = _steamVrPlayer.trackingOriginTransform.position - _steamVrPlayer.feetPositionGuess;
				_steamVrPlayer.trackingOriginTransform.position = teleportPosition + playerFeetOffset;
			}
			else
			{
