...
public class SampleDisplayer : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _segments = new Transform[LinkedPeer.NumberOfSamples];
        _segmentsMat = new Material[LinkedPeer.NumberOfSamples];
        
        for (int x = 0; x < LinkedPeer.NumberOfSamples; x++)
        {
            ...
            _segments[x].Rotate(transform.up, (CircleRepartition/LinkedPeer.NumberOfSamples)*x);
    ...
    private void Update()
    {
        for (int x = 0; x < LinkedPeer.NumberOfSamples; x++)
        {
            _segments[x].localScale = BaseSize + Direction * LinkedPeer.Samples[x] * MaxScale;
            _segmentsMat[x].SetFloat(_hashValue, LinkedPeer.Samples[x]);
        }
    }
}
