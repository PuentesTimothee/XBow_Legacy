...
public class BandDisplayer : MonoBehaviour
{
    ...
    private void Update()
    {
        for (int x = 0; x < LinkedPeer.NumberOfBands; x++)
        {
            _segments[x].localScale = BaseSize + Direction * LinkedPeer.Bands[x] * MaxScale;
            _segmentsMat[x].SetFloat(_hashValue, LinkedPeer.Bands[x]);
        }
    }
}
