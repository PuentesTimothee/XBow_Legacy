...
public class BasicController: MonoBehaviour {
	...
	void Update () {
		...
		if (isDead)
			...
			if (Input.GetKey (KeyCode.Space)) {
				...
				// IF mouseRotate boolean is set to true, THEN rotate character according to Horizontal mouse movement
						...
						anim.SetFloat ("zDeath", zDeath, transitionTime, Time.deltaTime);
						// Set xSpeed float as 'xSpeed' variable of the Animator, dampening it for the amount of time in 'transitionTime'
						anim.SetFloat ("xDeath", xDeath, transitionTime, Time.deltaTime);
			// }
		...
		var x = Input.GetAxis("Horizontal") * Time.deltaTime * 150.0f * speedLimit;
		var z = Input.GetAxis("Vertical") * Time.deltaTime * 7.0f *speedLimit;
